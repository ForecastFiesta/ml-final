...
train_set_x = np.load('./numpy_bin/train_set_x.npy').astype(np.float32)
train_set_y = np.load('./numpy_bin/train_set_y.npy').astype(np.float32)
valid_set_x = np.load('./numpy_bin/valid_set_x.npy').astype(np.float32)
valid_set_y = np.load('./numpy_bin/valid_set_y.npy').astype(np.float32)
...
def train(
    ...
    for epoch in range(epoch_cnt):
        ...
        if better_model:
            torch.save(model, f"./models/lstm-bidir-bs{BATCH_SIZE}-lr{LEARNING_RATE}.pt")
